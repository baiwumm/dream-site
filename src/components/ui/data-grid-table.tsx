...
import { Checkbox, type CheckboxProps } from '@/components/animate-ui/components/radix/checkbox';
...
function DataGridTableRowSelect<TData>({ row, size }: { row: Row<TData>; size?: CheckboxProps['size'] }) {
  return (
    <>
      <div
        className={cn('hidden absolute top-0 bottom-0 start-0 w-0.5 bg-primary', row.getIsSelected() && 'block')}
      ></div>
      <Checkbox
...
function DataGridTableRowSelectAll({ size }: { size?: CheckboxProps['size'] }) {
        ...
        {isLoading && props.loadingMode === 'skeleton' && pagination?.pageSize ? (
          ...
          <tr>
            <td colSpan={table.getVisibleFlatColumns().length}>
              <div className="flex flex-col items-center justify-center gap-1 min-h-50">
                <Spinner variant='circle' />
                <span>{props.loadingMessage || '加载中...'}</span>
              </div>
            </td>
          </tr>

<template>
  <el-table v-loading="pending" :data="dataSource" border stripe table-layou="auto">
    ...
    <el-table-column prop="name" label="站点名称" align="center" show-overflow-tooltip min-width="200">
    ...
    <el-table-column prop="desc" label="分类描述" align="center" show-overflow-tooltip min-width="160">
      <template #default="{ row }">
        {{ row.desc || "--" }}
      </template>
    </el-table-column>
    <el-table-column prop="desc" label="所属分类" align="center" show-overflow-tooltip min-width="160">
      <template #default="{ row }">
        <el-tag type="success">
    ...
    <el-table-column prop="created_at" label="创建时间" align="center" width="180" sortable>
      <template #default="{ row }">
        <div>{{ $dayjs(row.created_at).format("YYYY-MM-DD") }}</div>
      </template>
    </el-table-column>
    <el-table-column prop="updated_at" label="更新时间" align="center" width="180" sortable>
      <template #default="{ row }">
        <div>{{ $dayjs(row.updated_at).format("YYYY-MM-DD") }}</div>
      </template>
    </el-table-column>
...
import type { CategoryList } from "~/lib/type";
